...
export function updateSectionOffsets() {
    const heroSection = document.getElementById('hero') || document.querySelector('.hero-cinematic');
    if (heroSection) {
        // We type assert since we know these DOM properties exist in browser environments
        cachedHeroBottom = (heroSection as any).offsetTop + (heroSection as any).offsetHeight;
    }
}

export function handleScrollCache() {
    return cachedHeroBottom;
}
